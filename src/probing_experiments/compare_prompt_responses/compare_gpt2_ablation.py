...
def compare_gpt2_pew_token_pairs(cultures : list = None,
    ...
    save_dir = f'data/ablation/pew_{model_name}_token_pairs_{prompt_mode}.csv'
...
def compare_gpt2_wvs_token_pairs(cultures : list = None, wave = 7,model_name = 'gpt2',
    ...
    df = pd.DataFrame(gpt2_all)
    save_dir = f'data/ablation/wvs_w{wave}_{model_name}_token_pairs_{prompt_mode}.csv'
    df.to_csv(save_dir, index = False )
...
def compare_gpt2s():
    #store_small_pew()
    model_names = ['gpt2-medium','gpt-large']
    for mn in model_names:
        #compare_gpt2_pair_prompts_mort_user_study(style='mv_at_end', modelname=mn)
        compare_gpt2_wvs_token_pairs(DENONYM_WV7_LIST,model_name= mn, use_cuda=True, prompt_mode='in')
        compare_gpt2_pew_token_pairs(cultures=DENONYM_PEW_LIST, model_name=mn, use_cuda=True, prompt_mode = 'in')
