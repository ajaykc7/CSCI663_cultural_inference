...
from probing_experiments.wvs_prompts import \
    COUNTRIES_WVS_W7, DENONYM_WV7_LIST
...
def compare_mcm_pew(rating_path: str = '', saving_path: str = '', save_prompts = True):
    ...
    for root_culture,culture in DENONYM_DICT_PEW.items():
        ...
        for question,ratings in prompts.items():
            ...
            culture_pew_mcm = pew_mcms[root_culture]
    ...
    if save_prompts:
        pickle.dump(culture_prompts, open('data/PEW_2013/Ablation/compare_to_mcm.p', 'wb'))
...
def compare_wvs_mcm(rating_path: str = '',saving_path : str = '',save_prompts = True, wave = 7):
    ...
    culture_prompts = {}
    for root_culture,culture in DENONYM_DICT_WV7.items():
        ...
        for question,ratings in prompts.items():
            wvs_score = ratings[0]
            culture_pew_mcm = wvs_mcms[root_culture]
    ...
    if save_prompts:
        pickle.dump(culture_prompts, open(f'data/WVS/Ablation/compare_to_mcm_w{wave}.p', 'wb'))
...
def compare_mcm():
    compare_mcm_pew('data/PEW_2013/pew_mcm_all.p','data/ablation/pew_mcm.csv')
    compare_wvs_mcm('data/WVS/wvs_w7_mcm_all.p','data/ablation/wvs_w7_mcm.csv')
